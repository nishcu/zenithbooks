...
const formSchema = z.object({
  ...
  companyAddress: z.string().min(10, "Company's address is required."),
  
  exitingFounderName: z.string().min(3, "Exiting Founder name is required."),
  exitingFounderAddress: z.string().min(10, "Exiting Founder's address is required."),

  agreementDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: "Invalid date" }),
  
  exitTermsDescription: z.string().min(10, "A description of services is required."),
...
export default function CofounderExitAgreementPage() {
  ...
  useOnDemandUnlock("cofounder_exit_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'cofounder_exit_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["companyName", "companyAddress", "exitingFounderName", "exitingFounderAddress", "agreementDate"];
        break;
      case 2:
        fieldsToValidate = ["exitTermsDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Company and the Exiting Founder.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Company Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="companyName" render={({ field }) => ( <FormItem><FormLabel>Company Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="companyAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
                    <h3 className="text-lg font-semibold mb-2">Exiting Founder Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="exitingFounderName" render={({ field }) => ( <FormItem><FormLabel>Exiting Founder Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="exitingFounderAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                <Separator />
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Co-founder Exit Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Co-founder Exit Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.companyName}</strong>, with its principal office at {formData.companyAddress} (the "Company"),</p>
                    
                    <p className="font-bold">AND</p>
                    <p><strong>{formData.exitingFounderName}</strong>, with its principal office at {formData.exitingFounderAddress} (the "Exiting Founder").</p>

                    <h4 className="font-bold mt-4">1. Scope of Services</h4>
                    <p>The Company agrees to provide the following services to the Exiting Founder: {formData.exitTermsDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Exiting Founder agrees to pay the Company a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Company is an independent contractor and not an employee of the Exiting Founder.</p>
                    ...
                    <div className="grid grid-cols-2 gap-16 mt-16">
                        ...
                        <div className="text-left">
                            ...
                            <p className="mt-12">_________________________</p>
                            <p>By: {formData.exitingFounderName}</p>
                        </div>
                    </div>
                  </div>
                </CardContent>
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'cofounder_exit_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="cofounder_exit_agreement_download"
                        planName="Co-founder Exit Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.exitingFounderName}`}
                        contentRef={printRef}
                        documentType="cofounder_exit_agreement"
                        documentName={`Service_Agreement_${formData.exitingFounderName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Co-founder Exit Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Co-founder Exit Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'cofounder_exit_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'cofounder_exit_agreement')?.price || 0}
                planId="cofounder_exit_agreement_certification"
                planName="Co-founder Exit Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Co-founder Exit Agreement Certification",
                    exitingFounderName: form.getValues("exitingFounderName"),
