...
export default function TrademarkAuthorizationPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("trademark_authorization_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <Link href="/legal-documents/category/registration" className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" />
        Back to Registration Documents
      </Link>
      <div className="text-center">
        <h1 className="text-3xl font-bold">Trademark Registration Authorization Letter Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Authorize agent for trademark registration for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="trademark_authorization_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Trademark_Authorization_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="trademark_authorization"
                        documentName={`Trademark_Authorization_${formData.employeeName}`}
