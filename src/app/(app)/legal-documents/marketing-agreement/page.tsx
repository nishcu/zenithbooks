...
const formSchema = z.object({
  marketingAgencyName: z.string().min(3, "Marketing Agency name is required."),
  marketingAgencyAddress: z.string().min(10, "Marketing Agency's address is required."),
  ...
  marketingServicesDescription: z.string().min(10, "A description of services is required."),
...
export default function MarketingAgreementPage() {
  ...
  useOnDemandUnlock("marketing_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'marketing_agreement'
  });
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      marketingAgencyName: "",
      marketingAgencyAddress: "",
      clientName: "",
      clientAddress: "",
      agreementDate: "",
      marketingServicesDescription: "",
      paymentAmount: 10000,
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["marketingAgencyName", "marketingAgencyAddress", "clientName", "clientAddress", "agreementDate"];
        break;
      case 2:
        fieldsToValidate = ["marketingServicesDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Marketing Agency and the Client.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Marketing Agency Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="marketingAgencyName" render={({ field }) => ( <FormItem><FormLabel>Marketing Agency Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="marketingAgencyAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Marketing Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Marketing Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.marketingAgencyName}</strong>, with its principal office at {formData.marketingAgencyAddress} (the "Marketing Agency"),</p>
                    ...
                    <p>The Marketing Agency agrees to provide the following services to the Client: {formData.marketingServicesDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Client agrees to pay the Marketing Agency a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Marketing Agency is an independent contractor and not an employee of the Client.</p>
                    ...
                    <div className="grid grid-cols-2 gap-16 mt-16">
                        <div className="text-left">
                            ...
                            <p>By: {formData.marketingAgencyName}</p>
                        </div>
                        <div className="text-left">
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'marketing_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="marketing_agreement_download"
                        planName="Marketing Agreement Download"
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Marketing Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Marketing Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'marketing_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'marketing_agreement')?.price || 0}
                planId="marketing_agreement_certification"
                planName="Marketing Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Marketing Agreement Certification",
