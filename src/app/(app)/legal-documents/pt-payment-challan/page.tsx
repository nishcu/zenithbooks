...
export default function PTPaymentChallanPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("pt_payment_challan_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">PT Payment Challan Auto Draft Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Generate PT payment challan for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="pt_payment_challan_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`PT_Payment_Challan_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="pt_payment_challan"
                        documentName={`PT_Payment_Challan_${formData.employeeName}`}
