...
export default function VendorOnboardingKYCPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("vendor_onboarding_kyc_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Vendor Onboarding KYC Sheet Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          KYC form for vendor onboarding. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="vendor_onboarding_kyc_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Vendor_Onboarding_Kyc_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="vendor_onboarding_kyc"
                        documentName={`Vendor_Onboarding_Kyc_${formData.employeeName}`}
