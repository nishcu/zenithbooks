...
export default function HRPolicyHandbookPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("hr_policy_handbook_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">HR Policy Handbook Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Comprehensive employee handbook template. for your organization.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="hr_policy_handbook_download"
                        planName="Remote Work Policy Download"
                        amount={effectivePrice}
                        fileName={`Hr_Policy_Handbook_${formData.companyName}`}
                        contentRef={printRef}
                        documentType="hr_policy_handbook"
                        documentName={`Hr_Policy_Handbook_${formData.companyName}`}
