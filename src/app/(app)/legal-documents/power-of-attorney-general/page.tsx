...
export default function PowerOfAttorneyGeneralPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("power_of_attorney_general_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Power of Attorney (General) Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          General power of attorney template. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="power_of_attorney_general_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Power_Of_Attorney_General_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="power_of_attorney_general"
                        documentName={`Power_Of_Attorney_General_${formData.employeeName}`}
