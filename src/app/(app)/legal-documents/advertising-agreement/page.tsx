...
const formSchema = z.object({
  advertisingAgencyName: z.string().min(3, "Advertising Agency name is required."),
  advertisingAgencyAddress: z.string().min(10, "Advertising Agency's address is required."),
...
export default function AdvertisingAgreementPage() {
  ...
  useOnDemandUnlock("advertising_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'advertising_agreement'
  });
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      advertisingAgencyName: "",
      advertisingAgencyAddress: "",
      clientName: "",
      clientAddress: "",
      agreementDate: "",
      servicesDescription: "",
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["advertisingAgencyName", "advertisingAgencyAddress", "clientName", "clientAddress", "agreementDate"];
        break;
      case 2:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Advertising Agency and the Client.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Advertising Agency Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="advertisingAgencyName" render={({ field }) => ( <FormItem><FormLabel>Advertising Agency Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="advertisingAgencyAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Advertising Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Advertising Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.advertisingAgencyName}</strong>, with its principal office at {formData.advertisingAgencyAddress} (the "Advertising Agency"),</p>
                    ...
                    <p>The Advertising Agency agrees to provide the following services to the Client: {formData.servicesDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Client agrees to pay the Advertising Agency a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Advertising Agency is an independent contractor and not an employee of the Client.</p>
                    ...
                    <div className="grid grid-cols-2 gap-16 mt-16">
                        <div className="text-left">
                            ...
                            <p>By: {formData.advertisingAgencyName}</p>
                        </div>
                        <div className="text-left">
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'advertising_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="advertising_agreement_download"
                        planName="Advertising Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.clientName}`}
                        contentRef={printRef}
                        documentType="advertising_agreement"
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Advertising Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Advertising Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'advertising_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'advertising_agreement')?.price || 0}
                planId="advertising_agreement_certification"
                planName="Advertising Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Advertising Agreement Certification",
