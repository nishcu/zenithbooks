...
const formSchema = z.object({
  ...
  renewalTermsDescription: z.string().min(10, "A description of services is required."),
...
export default function FranchiseRenewalAgreementPage() {
  ...
  useOnDemandUnlock("franchise_renewal_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'franchise_renewal_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        ...
        break;
      case 2:
        fieldsToValidate = ["renewalTermsDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Franchise Renewal Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Franchise Renewal Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    ...
                    <p>The Franchisor agrees to provide the following services to the Franchisee: {formData.renewalTermsDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Franchisee agrees to pay the Franchisor a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Franchisor is an independent contractor and not an employee of the Franchisee.</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'franchise_renewal_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="franchise_renewal_agreement_download"
                        planName="Franchise Renewal Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.franchiseeName}`}
                        contentRef={printRef}
                        documentType="franchise_renewal_agreement"
                        documentName={`Service_Agreement_${formData.franchiseeName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Franchise Renewal Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Franchise Renewal Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'franchise_renewal_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'franchise_renewal_agreement')?.price || 0}
                planId="franchise_renewal_agreement_certification"
                planName="Franchise Renewal Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Franchise Renewal Agreement Certification",
                    franchiseeName: form.getValues("franchiseeName"),
