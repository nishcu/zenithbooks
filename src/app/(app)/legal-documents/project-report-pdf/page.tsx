...
export default function ProjectReportPDFPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("project_report_pdf_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Project Report PDF Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Generate detailed project reports for loans. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="project_report_pdf_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Project_Report_Pdf_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="project_report_pdf"
                        documentName={`Project_Report_Pdf_${formData.employeeName}`}
