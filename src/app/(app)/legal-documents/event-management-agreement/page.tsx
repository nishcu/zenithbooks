...
const formSchema = z.object({
  eventManagerName: z.string().min(3, "Event Manager name is required."),
  eventManagerAddress: z.string().min(10, "Event Manager's address is required."),
  ...
  eventServicesDescription: z.string().min(10, "A description of services is required."),
...
export default function EventManagementAgreementPage() {
  ...
  useOnDemandUnlock("event_management_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'event_management_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["eventManagerName", "eventManagerAddress", "clientName", "clientAddress", "agreementDate"];
        break;
      case 2:
        fieldsToValidate = ["eventServicesDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Event Manager and the Client.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Event Manager Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="eventManagerName" render={({ field }) => ( <FormItem><FormLabel>Event Manager Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="eventManagerAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Event Management Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Event Management Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.eventManagerName}</strong>, with its principal office at {formData.eventManagerAddress} (the "Event Manager"),</p>
                    ...
                    <p>The Event Manager agrees to provide the following services to the Client: {formData.eventServicesDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Client agrees to pay the Event Manager a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Event Manager is an independent contractor and not an employee of the Client.</p>
                    ...
                    <div className="grid grid-cols-2 gap-16 mt-16">
                        <div className="text-left">
                            ...
                            <p>By: {formData.eventManagerName}</p>
                        </div>
                        <div className="text-left">
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'event_management_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="event_management_agreement_download"
                        planName="Event Management Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.clientName}`}
                        contentRef={printRef}
                        documentType="event_management_agreement"
                        documentName={`Service_Agreement_${formData.clientName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Event Management Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Event Management Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'event_management_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'event_management_agreement')?.price || 0}
                planId="event_management_agreement_certification"
                planName="Event Management Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Event Management Agreement Certification",
