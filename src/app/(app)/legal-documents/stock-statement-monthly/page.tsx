...
export default function StockStatementMonthlyPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("stock_statement_monthly_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Stock Statement (Monthly) Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Monthly stock statement format for banks. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="stock_statement_monthly_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Stock_Statement_Monthly_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="stock_statement_monthly"
                        documentName={`Stock_Statement_Monthly_${formData.employeeName}`}
