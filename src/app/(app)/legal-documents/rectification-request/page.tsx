...
export default function RectificationRequestPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("rectification_request_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Rectification Request Draft Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Request for rectification of mistakes for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="rectification_request_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Rectification_Request_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="rectification_request"
                        documentName={`Rectification_Request_${formData.employeeName}`}
