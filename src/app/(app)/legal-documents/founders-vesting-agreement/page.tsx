...
export default function FoundersVestingAgreementPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("founders_vesting_agreement_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Founders Vesting Agreement Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Vesting schedule for founder shares for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="founders_vesting_agreement_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Founders_Vesting_Agreement_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="founders_vesting_agreement"
                        documentName={`Founders_Vesting_Agreement_${formData.employeeName}`}
