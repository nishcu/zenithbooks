...
export default function DebitCreditNotePage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("debit_credit_note_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Debit / Credit Note Format Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Debit and credit note templates. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="debit_credit_note_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Debit_Credit_Note_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="debit_credit_note"
                        documentName={`Debit_Credit_Note_${formData.employeeName}`}
