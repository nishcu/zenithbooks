...
export default function SalaryStructureLetterPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("salary_structure_letter_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Salary Structure Letter Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Generate salary structure breakdown letter for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="salary_structure_letter_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Salary_Structure_Letter_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="salary_structure_letter"
                        documentName={`Salary_Structure_Letter_${formData.employeeName}`}
