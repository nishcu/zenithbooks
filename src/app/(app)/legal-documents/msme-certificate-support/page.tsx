...
export default function MSMECertificateSupportPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("msme_certificate_support_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">MSME Certificate Support Documents Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Supporting documents for MSME certification for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="msme_certificate_support_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`MSME_Certificate_Support_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="msme_certificate_support"
                        documentName={`MSME_Certificate_Support_${formData.employeeName}`}
