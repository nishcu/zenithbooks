...
export default function GSTAmendmentApplicationPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("gst_amendment_application_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">GST Amendment Application Docs Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Documents for GST registration amendment for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="gst_amendment_application_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`GST_Amendment_Application_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="gst_amendment_application"
                        documentName={`GST_Amendment_Application_${formData.employeeName}`}
