...
const formSchema = z.object({
  ...
  outsourcedServicesDescription: z.string().min(10, "A description of services is required."),
...
export default function OutsourcingAgreementPage() {
  ...
  useOnDemandUnlock("outsourcing_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'outsourcing_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        ...
        break;
      case 2:
        fieldsToValidate = ["outsourcedServicesDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Outsourcing Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Outsourcing Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    ...
                    <p>The Service Provider agrees to provide the following services to the Client: {formData.outsourcedServicesDescription}</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'outsourcing_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="outsourcing_agreement_download"
                        planName="Outsourcing Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.clientName}`}
                        contentRef={printRef}
                        documentType="outsourcing_agreement"
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Outsourcing Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Outsourcing Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'outsourcing_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'outsourcing_agreement')?.price || 0}
                planId="outsourcing_agreement_certification"
                planName="Outsourcing Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Outsourcing Agreement Certification",
