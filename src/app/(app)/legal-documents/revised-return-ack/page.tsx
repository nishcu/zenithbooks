...
export default function RevisedReturnAckPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("revised_return_ack_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Revised Return Acknowledgment Format Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Format for revised return acknowledgment for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="revised_return_ack_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Revised_Return_Ack_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="revised_return_ack"
                        documentName={`Revised_Return_Ack_${formData.employeeName}`}
