...
export default function GSTCancellationAuthorizationPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("gst_cancellation_authorization_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <Link href="/legal-documents/category/tax" className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" />
        Back to Tax Documents
      </Link>
      <div className="text-center">
        <h1 className="text-3xl font-bold">GST Cancellation Authorization Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Authorize representative for GST cancellation for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="gst_cancellation_authorization_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`GST_Cancellation_Authorization_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="gst_cancellation_authorization"
                        documentName={`GST_Cancellation_Authorization_${formData.employeeName}`}
