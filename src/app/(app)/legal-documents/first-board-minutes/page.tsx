...
export default function FirstBoardMinutesPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("first_board_minutes_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">First Board Meeting Minutes Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Minutes template for first board meeting for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="first_board_minutes_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`First_Board_Minutes_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="first_board_minutes"
                        documentName={`First_Board_Minutes_${formData.employeeName}`}
