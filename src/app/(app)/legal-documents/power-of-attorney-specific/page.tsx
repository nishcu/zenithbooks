...
export default function PowerOfAttorneySpecificPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("power_of_attorney_specific_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Power of Attorney (Specific) Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Specific purpose power of attorney. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="power_of_attorney_specific_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Power_Of_Attorney_Specific_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="power_of_attorney_specific"
                        documentName={`Power_Of_Attorney_Specific_${formData.employeeName}`}
