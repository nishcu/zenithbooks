...
export default function ESOPGrantLetterPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("esop_grant_letter_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">ESOP Grant Letter Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Letter for granting employee stock options for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="esop_grant_letter_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`ESOP_Grant_Letter_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="esop_grant_letter"
                        documentName={`ESOP_Grant_Letter_${formData.employeeName}`}
