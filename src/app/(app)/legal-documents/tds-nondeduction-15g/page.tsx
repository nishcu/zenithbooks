...
export default function TDSNonDeduction15GPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("tds_nondeduction_15g_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">TDS Non-Deduction Declaration (15G/15H) Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Auto-fill Form 15G/15H for TDS exemption for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="tds_nondeduction_15g_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`TDS_NonDeduction_15G_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="tds_nondeduction_15g"
                        documentName={`TDS_NonDeduction_15G_${formData.employeeName}`}
