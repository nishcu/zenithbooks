...
const formSchema = z.object({
  ...
  serviceLevelDescription: z.string().min(10, "A description of services is required."),
...
export default function SLAAgreementPage() {
  ...
  useOnDemandUnlock("sla_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'sla_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        ...
        break;
      case 2:
        fieldsToValidate = ["serviceLevelDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated SLA Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This SLA Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    ...
                    <p>The Service Provider agrees to provide the following services to the Client: {formData.serviceLevelDescription}</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'sla_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="sla_agreement_download"
                        planName="SLA Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.clientName}`}
                        contentRef={printRef}
                        documentType="sla_agreement"
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">SLA Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your SLA Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'sla_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'sla_agreement')?.price || 0}
                planId="sla_agreement_certification"
                planName="SLA Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "SLA Agreement Certification",
