...
const formSchema = z.object({
  influencerName: z.string().min(3, "Influencer name is required."),
  influencerAddress: z.string().min(10, "Influencer's address is required."),
  
  brandName: z.string().min(3, "Brand name is required."),
  brandAddress: z.string().min(10, "Brand's address is required."),

  agreementDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: "Invalid date" }),
  
  influencerServicesDescription: z.string().min(10, "A description of services is required."),
...
export default function InfluencerAgreementPage() {
  ...
  useOnDemandUnlock("influencer_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'influencer_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["influencerName", "influencerAddress", "brandName", "brandAddress", "agreementDate"];
        break;
      case 2:
        fieldsToValidate = ["influencerServicesDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Influencer and the Brand.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Influencer Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="influencerName" render={({ field }) => ( <FormItem><FormLabel>Influencer Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="influencerAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
                    <h3 className="text-lg font-semibold mb-2">Brand Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="brandName" render={({ field }) => ( <FormItem><FormLabel>Brand Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="brandAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                <Separator />
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Influencer Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Influencer Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.influencerName}</strong>, with its principal office at {formData.influencerAddress} (the "Influencer"),</p>
                    
                    <p className="font-bold">AND</p>
                    <p><strong>{formData.brandName}</strong>, with its principal office at {formData.brandAddress} (the "Brand").</p>

                    <h4 className="font-bold mt-4">1. Scope of Services</h4>
                    <p>The Influencer agrees to provide the following services to the Brand: {formData.influencerServicesDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Brand agrees to pay the Influencer a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Influencer is an independent contractor and not an employee of the Brand.</p>
                    ...
                    <div className="grid grid-cols-2 gap-16 mt-16">
                        <div className="text-left">
                            ...
                            <p>By: {formData.influencerName}</p>
                        </div>
                        <div className="text-left">
                            <p className="font-bold">CLIENT:</p>
                            <p className="mt-12">_________________________</p>
                            <p>By: {formData.brandName}</p>
                        </div>
                    </div>
                  </div>
                </CardContent>
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'influencer_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="influencer_agreement_download"
                        planName="Influencer Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.brandName}`}
                        contentRef={printRef}
                        documentType="influencer_agreement"
                        documentName={`Service_Agreement_${formData.brandName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Influencer Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Influencer Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'influencer_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'influencer_agreement')?.price || 0}
                planId="influencer_agreement_certification"
                planName="Influencer Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Influencer Agreement Certification",
