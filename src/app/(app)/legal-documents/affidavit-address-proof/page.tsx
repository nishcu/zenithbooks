...
export default function AffidavitAddressProofPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("affidavit_address_proof_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Address Proof Affidavit Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Affidavit for address proof purposes. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="affidavit_address_proof_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Affidavit_Address_Proof_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="affidavit_address_proof"
                        documentName={`Affidavit_Address_Proof_${formData.employeeName}`}
