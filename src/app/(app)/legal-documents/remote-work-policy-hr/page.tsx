...
export default function RemoteWorkPolicyHRPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("remote_work_policy_hr_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Remote Work Policy Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Create a comprehensive remote work policy for your organization.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="remote_work_policy_hr_download"
                        planName="Remote Work Policy Download"
                        amount={effectivePrice}
                        fileName={`Remote_Work_Policy_${formData.companyName}`}
                        contentRef={printRef}
                        documentType="remote_work_policy_hr"
                        documentName={`Remote_Work_Policy_${formData.companyName}`}
