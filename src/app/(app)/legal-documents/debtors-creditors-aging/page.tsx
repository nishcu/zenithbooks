...
export default function DebtorsCreditorsAgingPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("debtors_creditors_aging_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Debtors / Creditors Aging Certificate Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Aging analysis certificate for bank compliance. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="debtors_creditors_aging_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Debtors_Creditors_Aging_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="debtors_creditors_aging"
                        documentName={`Debtors_Creditors_Aging_${formData.employeeName}`}
