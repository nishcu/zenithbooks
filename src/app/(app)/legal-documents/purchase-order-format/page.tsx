...
export default function PurchaseOrderFormatPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("purchase_order_format_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Purchase Order Format Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Standard purchase order template. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="purchase_order_format_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Purchase_Order_Format_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="purchase_order_format"
                        documentName={`Purchase_Order_Format_${formData.employeeName}`}
