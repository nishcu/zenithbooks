...
const formSchema = z.object({
  ...
  dealerAddress: z.string().min(10, "Dealer's address is required."),
...
export default function DealerAgreementPage() {
  ...
  useOnDemandUnlock("dealer_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'dealer_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["dealerName", "dealerAddress", "supplierName", "supplierAddress", "agreementDate"];
        break;
      case 2:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Dealer and the Supplier.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Dealer Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="dealerName" render={({ field }) => ( <FormItem><FormLabel>Dealer Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="dealerAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Dealer Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Dealer Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.dealerName}</strong>, with its principal office at {formData.dealerAddress} (the "Dealer"),</p>
                    ...
                    <p>The Dealer agrees to provide the following services to the Supplier: {formData.productsDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Supplier agrees to pay the Dealer a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Products. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Dealer is an independent contractor and not an employee of the Supplier.</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'dealer_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="dealer_agreement_download"
                        planName="Dealer Agreement Download"
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Dealer Agreement Generator</h1>
        <p className="text-muted-foreground">Define dealer relationship and sales terms.</p>
      </div>
      <Form {...form}>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Dealer Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'dealer_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'dealer_agreement')?.price || 0}
                planId="dealer_agreement_certification"
                planName="Dealer Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Dealer Agreement Certification",
