...
const formSchema = z.object({
  ...
  shareQuantity: z.string().min(8, "A valid DIN is required."),
  effectiveDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: "Invalid date" }),
});

type FormData = z.infer<typeof formSchema>;

export default function AllotmentSharesPage() {
  ...
  useOnDemandUnlock("allotment_shares_download", () => setShowDocument(true));
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Board_Resolution_Allottee_Appointment_${formData.companyName}`,
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Board Resolution: Allotment of Shares</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Generate a board resolution to allot shares to the board.
        </p>
      </div>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <Form {...form}>
              <form className="space-y-4">
                 ...
                 <FormField control={form.control} name="allotteeName" render={({ field }) => (<FormItem><FormLabel>New Allottee's Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)}/>
                 <FormField control={form.control} name="shareQuantity" render={({ field }) => (<FormItem><FormLabel>New Allottee's DIN</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)}/>
            ...
            <Card>
                ...
                <CardContent ref={printRef} className="p-8 border-dashed border-2 rounded-lg prose prose-sm dark:prose-invert max-w-none">
                    ...
                    <div className="mt-8 text-justify">
                        ...
                        <p><strong className="uppercase">RESOLVED THAT</strong> pursuant to the provisions of Section 161 of the Companies Act, 2013, and the Articles of Association of the Company, <strong>{formData.allotteeName || '[Allottee Name]'}</strong> (DIN: <strong>{formData.shareQuantity || '[DIN]'}</strong>), who has signified her/his consent to act as a Allottee of the Company, be and is hereby appointed as an Additional Allottee of the Company with effect from <strong>{formData.effectiveDate ? new Date(formData.effectiveDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric'}) : '[Date]'}</strong>.</p>
                        <p><strong className="uppercase">RESOLVED FURTHER THAT</strong> {formData.allotteeName || '[Allottee Name]'} shall hold office up to the date of the next Annual General Meeting of the Company.</p>
                        <p><strong className="uppercase">RESOLVED FURTHER THAT</strong> any Allottee of the Company be and is hereby authorized to file the necessary forms with the Registrar of Companies and to do all such acts, deeds and things as may be necessary to give effect to this resolution.</p>
                    </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="allotment_shares_download"
                        planName="Board Resolution Download"
                        amount={effectivePrice}
                        fileName={`Board_Resolution_Allottee_Appointment_${formData.companyName}`}
                        contentRef={printRef}
                        documentType="board_resolutions"
                        documentName={`Board_Resolution_Allottee_Appointment_${formData.companyName}`}
