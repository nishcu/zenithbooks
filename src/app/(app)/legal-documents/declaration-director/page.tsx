...
export default function DeclarationDirectorPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("declaration_director_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Declaration of Director Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Director eligibility and compliance declaration for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="declaration_director_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Declaration_Director_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="declaration_director"
                        documentName={`Declaration_Director_${formData.employeeName}`}
