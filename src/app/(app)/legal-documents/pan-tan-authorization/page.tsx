...
export default function PANTANAuthorizationPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("pan_tan_authorization_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">PAN / TAN Application Authorization Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Authorize representative for PAN/TAN applications for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="pan_tan_authorization_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`PAN_TAN_Authorization_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="pan_tan_authorization"
                        documentName={`PAN_TAN_Authorization_${formData.employeeName}`}
