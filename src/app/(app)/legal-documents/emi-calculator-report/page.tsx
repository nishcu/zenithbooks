...
export default function EMICalculatorReportPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("emi_calculator_report_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">EMI Calculator Report Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Loan EMI calculation and amortization report. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="emi_calculator_report_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Emi_Calculator_Report_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="emi_calculator_report"
                        documentName={`Emi_Calculator_Report_${formData.employeeName}`}
