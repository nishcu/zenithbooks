...
export default function AngelInvestmentAgreementPage() {
  ...
  useOnDemandUnlock("angel_investment_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'angel_investment_agreement'
  });
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Angel Investment Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Angel Investment Agreement is made as of <strong>{agreementDate}</strong>,</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'angel_investment_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="angel_investment_agreement_download"
                        planName="Angel Investment Agreement Download"
                        amount={effectivePrice}
                        fileName={`Angel_Investment_Agreement_${formData.clientName}`}
                        contentRef={printRef}
                        documentType="angel_investment_agreement"
                        documentName={`Angel_Investment_Agreement_${formData.clientName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Angel Investment Agreement Generator</h1>
        <p className="text-muted-foreground">A Agreement for angel investor funding.</p>
      </div>
      <Form {...form}>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Angel Investment Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'angel_investment_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'angel_investment_agreement')?.price || 0}
                planId="angel_investment_agreement_certification"
                planName="Angel Investment Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Angel Investment Agreement Certification",
