...
const formSchema = z.object({
  ...
  contractorName: z.string().min(3, "Contractor/Freelancer name is required."),
  contractorAddress: z.string().min(10, "Contractor's address is required."),
  ...
  confidentiality: z.string().default("The Contractor shall maintain the confidentiality of all of the Client's proprietary and confidential information and shall not disclose it to any third party without the Client's prior written consent."),
  ipOwnership: z.string().default("Any intellectual property created by the Contractor in the performance of the Services will be the sole and exclusive property of the Client."),
...
export default function ContractorAgreementPage() {
  ...
  useOnDemandUnlock("contractor_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'contractor_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["clientName", "clientAddress", "contractorName", "contractorAddress", "agreementDate"];
        break;
      case 2:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Client and the Contractor.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                 ...
                 <div>
                    <h3 className="text-lg font-semibold mb-2">Contractor / Freelancer Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="contractorName" render={({ field }) => ( <FormItem><FormLabel>Name (Individual or Company)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="contractorAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                <Separator />
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Contractor / Freelancer Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    <h2 className="text-center font-bold">CONTRACTOR / FREELANCER AGREEMENT</h2>
                    ...
                    <p><strong>{formData.contractorName}</strong>, having its principal place of business at {formData.contractorAddress} (the "Contractor").</p>

                    <h4 className="font-bold mt-4">1. Services</h4>
                    <p>The Contractor agrees to perform the following services for the Client: {formData.servicesDescription}</p>

                    <h4 className="font-bold mt-4">2. Compensation</h4>
                    <p>In consideration for the Services, the Client will pay the Contractor a fee of ₹{formData.paymentAmount.toLocaleString('en-IN')}. Payment shall be made according to the following terms: {formData.paymentTerms}.</p>
                    
                    <h4 className="font-bold mt-4">3. Term and Termination</h4>
                    <p>{formData.term}</p>

                    <h4 className="font-bold mt-4">4. Independent Contractor Status</h4>
                    <p>The Contractor is an independent contractor, and nothing in this Agreement shall be construed to create a partnership, joint venture, or employer-employee relationship.</p>
                    ...
                    <div className="grid grid-cols-2 gap-16 mt-16">
                        ...
                        <div className="text-left">
                            ...
                            <p className="mt-12">_________________________</p>
                            <p>By: {formData.contractorName}</p>
                        </div>
                    </div>
                  </div>
                </CardContent>
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'contractor_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="contractor_agreement_download"
                        planName="Contractor Agreement Download"
                        amount={effectivePrice}
                        fileName={`Contractor_Agreement_${formData.contractorName}`}
                        contentRef={printRef}
                        documentType="contractor_agreement"
                        documentName={`Contractor_Agreement_${formData.contractorName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Contractor / Freelancer Agreement</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Contractor Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Our legal experts will review your contractor agreement for legal compliance, ensure all necessary clauses are included, and provide certification for legal validity.
            </p>
          </CardContent>
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'contractor_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'contractor_agreement')?.price || 0}
                planId="contractor_agreement_certification"
                planName="Contractor Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Contractor Agreement Certification",
