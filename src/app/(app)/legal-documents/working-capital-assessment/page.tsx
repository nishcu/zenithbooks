...
export default function WorkingCapitalAssessmentPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("working_capital_assessment_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <Link href="/legal-documents/category/finance-banking" className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" />
        Back to Finance & Banking Documents
      </Link>
      <div className="text-center">
        <h1 className="text-3xl font-bold">Working Capital Assessment Document Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Assessment document for working capital loan. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="working_capital_assessment_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Working_Capital_Assessment_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="working_capital_assessment"
                        documentName={`Working_Capital_Assessment_${formData.employeeName}`}
