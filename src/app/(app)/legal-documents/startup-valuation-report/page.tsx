...
export default function StartupValuationReportPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("startup_valuation_report_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Startup Valuation Report Template Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Template for startup valuation reports for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="startup_valuation_report_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Startup_Valuation_Report_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="startup_valuation_report"
                        documentName={`Startup_Valuation_Report_${formData.employeeName}`}
