...
export default function NonCompeteAgreementPage() {
  ...
  useOnDemandUnlock("non_compete_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'non_compete_agreement'
  });
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 3:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Non-Compete Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    <h2 className="text-center font-bold">NON-COMPETE AGREEMENT</h2>
                    
                    <p>This Non-Compete Agreement (the "Agreement") is entered into as of <strong>{agreementDate}</strong> (the "Effective Date"),</p>
                    ...
                    <h4 className="font-bold mt-4">1. Non-Compete Restriction</h4>
                    <p>During the term of employment and for a period of <strong>{formData.restrictionYears} year(s)</strong> following the termination of employment (the "Restricted Period"), the Employee agrees not to directly or indirectly engage in, own, manage, operate, control, or participate in any business that competes with the Employer's business as described: {formData.businessDescription}.</p>
                    ...
                    <h4 className="font-bold mt-4">3. Prohibited Activities</h4>
                    <p>During the Restricted Period, the Employee shall not: (a) engage in any competitive business activity; (b) solicit or accept business from any client, customer, or account of the Employer; (c) hire or attempt to hire any employee of the Employer; or (d) use or disclose any confidential information of the Employer.</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'non_compete_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="non_compete_agreement_download"
                        planName="Non-Compete Agreement Download"
                        amount={effectivePrice}
                        fileName={`Non-Compete Agreement_${formData.employerName}_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="non_compete_agreement"
                        documentName={`Non-Compete Agreement_${formData.employerName}_${formData.employeeName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Non-Compete Agreement Generator</h1>
        <p className="text-muted-foreground">Follow the steps to create a non-compete agreement restricting employees from competing during and after employment.</p>
      </div>
      <Form {...form}>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Non-Compete Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {(() => {
              ...
              <CashfreeCheckout
                amount={effectivePrice}
                planId="non_compete_agreement_certification"
                planName="Non-Compete Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Non-Compete Agreement Certification",
