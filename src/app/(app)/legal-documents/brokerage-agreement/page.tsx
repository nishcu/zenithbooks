...
const formSchema = z.object({
  brokerName: z.string().min(3, "Broker name is required."),
  brokerAddress: z.string().min(10, "Broker's address is required."),
  ...
  brokerageServicesDescription: z.string().min(10, "A description of services is required."),
...
export default function BrokerageAgreementPage() {
  ...
  useOnDemandUnlock("brokerage_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'brokerage_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["brokerName", "brokerAddress", "clientName", "clientAddress", "agreementDate"];
        break;
      case 2:
        fieldsToValidate = ["brokerageServicesDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Broker and the Client.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Broker Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="brokerName" render={({ field }) => ( <FormItem><FormLabel>Broker Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="brokerAddress" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Brokerage Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Brokerage Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.brokerName}</strong>, with its principal office at {formData.brokerAddress} (the "Broker"),</p>
                    ...
                    <p>The Broker agrees to provide the following services to the Client: {formData.brokerageServicesDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Client agrees to pay the Broker a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Broker is an independent contractor and not an employee of the Client.</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'brokerage_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="brokerage_agreement_download"
                        planName="Brokerage Agreement Download"
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Brokerage Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Brokerage Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'brokerage_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'brokerage_agreement')?.price || 0}
                planId="brokerage_agreement_certification"
                planName="Brokerage Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Brokerage Agreement Certification",
