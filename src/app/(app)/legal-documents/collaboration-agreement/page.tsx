...
const formSchema = z.object({
  party1Name: z.string().min(3, "Party 1 name is required."),
  party1Address: z.string().min(10, "Party 1's address is required."),
  
  party2Name: z.string().min(3, "Party 2 name is required."),
  party2Address: z.string().min(10, "Party 2's address is required."),

  agreementDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: "Invalid date" }),
  
  collaborationDescription: z.string().min(10, "A description of services is required."),
...
export default function CollaborationAgreementPage() {
  ...
  useOnDemandUnlock("collaboration_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'collaboration_agreement'
  });
  ...
  const processStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ["party1Name", "party1Address", "party2Name", "party2Address", "agreementDate"];
        break;
      case 2:
        fieldsToValidate = ["collaborationDescription", "paymentAmount", "paymentTerms"];
        break;
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <Card>
            <CardHeader><CardTitle>Step 1: Parties & Date</CardTitle><CardDescription>Enter details about the Party 1 and the Party 2.</CardDescription></CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <h3 className="text-lg font-semibold mb-2">Party 1 Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="party1Name" render={({ field }) => ( <FormItem><FormLabel>Party 1 Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="party1Address" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                 <div>
                    <h3 className="text-lg font-semibold mb-2">Party 2 Details</h3>
                    <div className="space-y-4 p-4 border rounded-lg">
                        <FormField control={form.control} name="party2Name" render={({ field }) => ( <FormItem><FormLabel>Party 2 Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )}/>
                        <FormField control={form.control} name="party2Address" render={({ field }) => ( <FormItem><FormLabel>Address</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem> )}/>
                    </div>
                </div>
                <Separator />
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Collaboration Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Collaboration Agreement is made as of <strong>{agreementDate}</strong>,</p>
                    
                    <p className="font-bold">BY AND BETWEEN:</p>
                    <p><strong>{formData.party1Name}</strong>, with its principal office at {formData.party1Address} (the "Party 1"),</p>
                    
                    <p className="font-bold">AND</p>
                    <p><strong>{formData.party2Name}</strong>, with its principal office at {formData.party2Address} (the "Party 2").</p>

                    <h4 className="font-bold mt-4">1. Scope of Services</h4>
                    <p>The Party 1 agrees to provide the following services to the Party 2: {formData.collaborationDescription}</p>

                    <h4 className="font-bold mt-4">2. Payment</h4>
                    <p>The Party 2 agrees to pay the Party 1 a total fee of ₹{formData.paymentAmount.toLocaleString('en-IN')} for the Services. Payment shall be made as follows: {formData.paymentTerms}.</p>
                    ...
                    <p>The Party 1 is an independent contractor and not an employee of the Party 2.</p>
                    ...
                    <div className="grid grid-cols-2 gap-16 mt-16">
                        <div className="text-left">
                            ...
                            <p>By: {formData.party1Name}</p>
                        </div>
                        <div className="text-left">
                            <p className="font-bold">CLIENT:</p>
                            <p className="mt-12">_________________________</p>
                            <p>By: {formData.party2Name}</p>
                        </div>
                    </div>
                  </div>
                </CardContent>
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'collaboration_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="collaboration_agreement_download"
                        planName="Collaboration Agreement Download"
                        amount={effectivePrice}
                        fileName={`Service_Agreement_${formData.party2Name}`}
                        contentRef={printRef}
                        documentType="collaboration_agreement"
                        documentName={`Service_Agreement_${formData.party2Name}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Collaboration Agreement Generator</h1>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Collaboration Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'collaboration_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'collaboration_agreement')?.price || 0}
                planId="collaboration_agreement_certification"
                planName="Collaboration Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Collaboration Agreement Certification",
                    party2Name: form.getValues("party2Name"),
