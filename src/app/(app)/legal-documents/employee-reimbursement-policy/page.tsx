...
export default function EmployeeReimbursementPolicyPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("employee_reimbursement_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Employee Reimbursement Policy Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Policy for expense reimbursements. for your organization.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="employee_reimbursement_download"
                        planName="Remote Work Policy Download"
                        amount={effectivePrice}
                        fileName={`Employee_Reimbursement_${formData.companyName}`}
                        contentRef={printRef}
                        documentType="employee_reimbursement"
                        documentName={`Employee_Reimbursement_${formData.companyName}`}
