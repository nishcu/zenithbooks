...
export default function AffidavitNameChangePage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("affidavit_name_change_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Name Change Affidavit Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Affidavit for name change applications. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="affidavit_name_change_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Affidavit_Name_Change_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="affidavit_name_change"
                        documentName={`Affidavit_Name_Change_${formData.employeeName}`}
