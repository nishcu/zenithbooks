...
export default function DeclarationSubscribersPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("declaration_subscribers_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <Link href="/legal-documents/category/company-law" className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" />
        Back to Company Law Documents
      </Link>
      <div className="text-center">
        <h1 className="text-3xl font-bold">Declaration by Subscribers Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Initial declaration for company formation for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="declaration_subscribers_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Declaration_Subscribers_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="declaration_subscribers"
                        documentName={`Declaration_Subscribers_${formData.employeeName}`}
