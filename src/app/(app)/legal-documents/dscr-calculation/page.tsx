...
export default function DSCRCalculationPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("dscr_calculation_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">DSCR Calculation Sheet Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Debt Service Coverage Ratio calculation template. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="dscr_calculation_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Dscr_Calculation_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="dscr_calculation"
                        documentName={`Dscr_Calculation_${formData.employeeName}`}
