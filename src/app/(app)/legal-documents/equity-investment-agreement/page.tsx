...
export default function EquityInvestmentAgreementPage() {
  ...
  useOnDemandUnlock("equity_investment_agreement_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'equity_investment_agreement'
  });
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Equity Investment Agreement.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Equity Investment Agreement is made as of <strong>{agreementDate}</strong>,</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'equity_investment_agreement')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="equity_investment_agreement_download"
                        planName="Equity Investment Agreement Download"
                        amount={effectivePrice}
                        fileName={`Equity_Investment_Agreement_${formData.clientName}`}
                        contentRef={printRef}
                        documentType="equity_investment_agreement"
                        documentName={`Equity_Investment_Agreement_${formData.clientName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      <Link href="/legal-documents/category/startup-funding" className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" />
        Back to Startup & Funding Documents
      </Link>
      <div className="text-center">
        <h1 className="text-3xl font-bold">Equity Investment Agreement Generator</h1>
        <p className="text-muted-foreground">A Agreement for equity investment rounds.</p>
      </div>
      <Form {...form}>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Equity Investment Agreement professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'equity_investment_agreement')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'equity_investment_agreement')?.price || 0}
                planId="equity_investment_agreement_certification"
                planName="Equity Investment Agreement Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Equity Investment Agreement Certification",
