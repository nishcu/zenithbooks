...
export default function BankLoanApplicationPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("bank_loan_application_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Bank Loan Application Template Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Comprehensive loan application template. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="bank_loan_application_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Bank_Loan_Application_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="bank_loan_application"
                        documentName={`Bank_Loan_Application_${formData.employeeName}`}
