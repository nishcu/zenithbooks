...
export default function TermSheetPage() {
  ...
  useOnDemandUnlock("term_sheet_download", () => setShowDocument(true));

  const { handleCertificationRequest, handlePaymentSuccess, isSubmitting: isCertifying } = useCertificationRequest({
    pricing,
    serviceId: 'term_sheet'
  });
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 4:
        ...
        return (
             <Card>
                <CardHeader><CardTitle>Final Step: Preview & Download</CardTitle><CardDescription>Review the generated Term Sheet.</CardDescription></CardHeader>
                <CardContent>
                  <div ref={printRef} className="prose prose-sm dark:prose-invert max-w-none border rounded-md p-6 bg-muted/20 leading-relaxed">
                    ...
                    <p>This Term Sheet is made as of <strong>{agreementDate}</strong>,</p>
                ...
                <CardFooter className="justify-between mt-6">
                  ...
                  {(() => {
                    const basePrice = pricing?.agreements?.find(s => s.id === 'term_sheet')?.price || 0;
                    ...
                    return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="term_sheet_download"
                        planName="Term Sheet Download"
                        amount={effectivePrice}
                        fileName={`Term_Sheet_${formData.clientName}`}
                        contentRef={printRef}
                        documentType="term_sheet"
                        documentName={`Term_Sheet_${formData.clientName}`}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Term Sheet Generator</h1>
        <p className="text-muted-foreground">A Investment term sheet template.</p>
      </div>
      <Form {...form}>
      ...
      {step === 3 && (
        <Card className="mt-8">
          <CardHeader>
            ...
            <CardDescription>Get your Term Sheet professionally reviewed and certified by a qualified legal expert.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            {pricing && pricing.agreements?.find(s => s.id === 'term_sheet')?.price > 0 ? (
              <CashfreeCheckout
                amount={pricing.agreements.find(s => s.id === 'term_sheet')?.price || 0}
                planId="term_sheet_certification"
                planName="Term Sheet Professional Certification"
                userId={user?.uid || ''}
                userEmail={user?.email || ''}
                userName={user?.displayName || ''}
                onSuccess={(paymentId) => {
                  handlePaymentSuccess(paymentId, {
                    reportType: "Term Sheet Certification",
