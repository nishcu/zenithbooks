...
export default function GSTRevocationRequestPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("gst_revocation_request_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">GST Revocation Request Letter Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Request revocation of cancelled GST registration for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="gst_revocation_request_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`GST_Revocation_Request_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="gst_revocation_request"
                        documentName={`GST_Revocation_Request_${formData.employeeName}`}
