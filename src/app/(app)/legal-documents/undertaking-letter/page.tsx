...
export default function UndertakingLetterPage() {
  const printRef = useRef(null);
  const [user] = useAuthState(auth);
  const [pricing, setPricing] = useState(null);
  const [userSubscriptionInfo, setUserSubscriptionInfo] = useState<{ userType: "business" | "professional" | null; subscriptionPlan: "freemium" | "business" | "professional" | null } | null>(null);
  const [showDocument, setShowDocument] = useState(false);
  useOnDemandUnlock("undertaking_letter_download", () => setShowDocument(true));
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Undertaking Letter Generator</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">
          General undertaking letter template. for employee misconduct or performance issues.
        </p>
      </div>
                ...
                {(() => {
                  ...
                  return (
                      <OnDemandPayAndUseActions
                        ...
                        planId="undertaking_letter_download"
                        planName="Warning Letter Download"
                        amount={effectivePrice}
                        fileName={`Undertaking_Letter_${formData.employeeName}`}
                        contentRef={printRef}
                        documentType="undertaking_letter"
                        documentName={`Undertaking_Letter_${formData.employeeName}`}
