...
const formSchema = z.object({
  ...
  investmentInPlantMachinery: z.coerce.number().positive("Investment must be positive."),
  turnoverPreviousFY: z.coerce.number().positive("Turnover must be positive."),
  msmeClassification: z.enum(["Micro", "Small", "Medium"]),
});
...
export default function MSMEInvestmentTurnoverCertificatePage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      documentName: `MSME Investment & Turnover Certificate - ${new Date().toISOString().split("T")[0]}`,
      entityName: "",
      fatherOrSpouseName: "",
      entityAddress: "",
      entityPan: "",
      ...
      investmentInPlantMachinery: 0,
      turnoverPreviousFY: 0,
        ...
        const loadDocument = async () => {
            ...
            if (docSnap.exists()) {
                ...
                if(data.userId === user.uid) {
                    ...
                    router.push('/ca-certificates/msme-investment-turnover');
                }
            } else {
      ...
      try {
          if (docId) {
              ...
              const docRef = await addDoc(collection(db, 'userDocuments'), {
                  userId: user.uid,
                  documentType: 'msme-investment-turnover-certificate',
              ...
              router.push(`/ca-certificates/msme-investment-turnover?id=${docRef.id}`);
          }
      } catch (e) {
          console.error(e);
  ...
  const handleLocalCertificationRequest = async () => {
      ...
      const basePrice = pricing?.ca_certs?.find(s => s.id === 'msme_investment_turnover')?.price || 0;
      ...
      if (effectivePrice === 0) {
          ...
          try {
            await addDoc(collection(db, "certificationRequests"), {
                reportType: "MSME Investment & Turnover Certificate",
  ...
  const renderContent = () => {
    ...
    if (step === 1) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle>MSME Investment & Turnover Information</CardTitle>
                </CardHeader>
                 <CardContent className="space-y-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                         ...
                         <FormField control={form.control} name="udyamRegistrationNumber" render={({ field }) => (<FormItem><FormLabel>Udyam Registration Number (Optional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)}/>
                    </div>
                    <div className="grid md:grid-cols-2 gap-4">
                         ...
                         <FormField control={form.control} name="msmeClassification" render={({ field }) => (
                            <FormItem>
                    ...
                    <Separator/>
                    <h4 className="font-semibold">Investment & Turnover Details (₹)</h4>
                     <FormField control={form.control} name="investmentInPlantMachinery" render={({ field }) => (<FormItem><FormLabel>Investment in Plant & Machinery (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="turnoverPreviousFY" render={({ field }) => (<FormItem><FormLabel>Turnover of Previous Financial Year (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                </CardContent>
    ...
    if (step === 2) {
        ...
        const whatsappMessage = `Dear ${formData.entityName},\n\nPlease find attached the MSME Investment & Turnover Certificate for FY ${formData.financialYear}.\n\nThank you,\n${CA_FIRM.name}`;
             ...
             <Card>
                ...
                <CardContent>
                     <div ref={printRef} className="prose dark:prose-invert max-w-none border rounded-lg p-8 bg-card text-card-foreground">
                        ...
                        <h4 className="font-bold text-center underline my-6">MSME INVESTMENT & TURNOVER CERTIFICATE</h4>

                        <p>This is to certify that we have verified the books of accounts and other relevant records of <strong>M/s {formData.entityName}</strong>{formData.fatherOrSpouseName ? ` (${formData.fatherOrSpouseName})` : ''}, having its registered office at {formData.entityAddress} and holding PAN <strong>{formData.entityPan}</strong>{formData.udyamRegistrationNumber ? ` and Udyam Registration Number ${formData.udyamRegistrationNumber}` : ''}.</p>
                        
                        <p>Based on our verification for the financial year {formData.financialYear}, we certify that the investment in plant & machinery and turnover details are as follows:</p>
                        ...
                        <table className="my-6 w-full">
                            <tbody>
                                ...
                                <tr className="border-b">
                                    <td className="py-2 font-semibold">Investment in Plant & Machinery</td>
                                    <td className="py-2 text-right font-mono font-semibold">₹ {formData.investmentInPlantMachinery.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                                <tr className="border-b">
                                    <td className="py-2 font-semibold">Turnover of Previous Financial Year</td>
                                    <td className="py-2 text-right font-mono font-semibold">₹ {formData.turnoverPreviousFY.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                                <tr className="border-t-2">
                                    <td className="py-2 font-semibold">MSME Classification</td>
                                    <td className="py-2 text-right font-semibold">{formData.msmeClassification} Enterprise</td>
                                </tr>
                            </tbody>
                        </table>

                        <p>The investment in plant & machinery is <strong>Rupees {numberToWords(formData.investmentInPlantMachinery)} only</strong>.</p>
                        <p>The turnover of previous financial year is <strong>Rupees {numberToWords(formData.turnoverPreviousFY)} only</strong>.</p>
                        <p>Based on the above figures, the entity is classified as a <strong>{formData.msmeClassification} Enterprise</strong> as per the MSME Act, 2006.</p>

                        <p className="mt-8 text-xs">
                            This certificate is issued at the specific request of the entity for the purpose of [Purpose, e.g., Bank Loan, Government Tender, etc.]. Our liability is limited to the extent of information provided by the management and is based on the records produced before us.
                        </p>
                ...
                <CardFooter className="justify-between">
                     ...
                     <div className="flex gap-2">
                        <ShareButtons
                            contentRef={printRef}
                            fileName={`MSME_Investment_Turnover_Certificate_${formData.entityName}`}
                        ...
                        {(() => {
                          ...
                          return effectivePrice > 0 ? (
                           <CashfreeCheckout
                               ...
                               planId="msme_investment_turnover_cert"
                               planName="MSME Investment & Turnover Certificate"
                               userId={user?.uid || ''}
                               ...
                               postPaymentContext={{
                                 ...
                                 payload: {
                                   ...
                                   clientName: form.getValues("entityName"),
                                   documentType: "msme-investment-turnover-certificate",
                                   documentName: form.getValues("documentName") || "MSME Investment & Turnover Certificate",
                                   formData: form.getValues(),
                                 },
                               }}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">MSME Investment & Turnover Certificate</h1>
        <p className="text-muted-foreground">Generate a draft certificate for MSME classification based on investment and turnover.</p>
      </div>
      <Form {...form}>
