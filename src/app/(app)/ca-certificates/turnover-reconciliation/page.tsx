...
const formSchema = z.object({
  ...
  gstr1Turnover: z.coerce.number().min(0, "Turnover cannot be negative."),
  gstr3bTurnover: z.coerce.number().min(0, "Turnover cannot be negative."),
  booksTurnover: z.coerce.number().min(0, "Turnover cannot be negative."),
  variance: z.coerce.number(),
  reasonForVariance: z.string().min(3, "Reason for variance is required."),
});
...
export default function TurnoverReconciliationCertificatePage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      documentName: `Turnover Reconciliation Certificate - ${new Date().toISOString().split("T")[0]}`,
      entityName: "",
      fatherOrSpouseName: "",
      entityAddress: "",
      entityPan: "",
      ...
      booksTurnover: 0,
      variance: 0,
      reasonForVariance: "Timing differences, export sales, advances, etc.",
    },
  });
        ...
        const loadDocument = async () => {
            ...
            if (docSnap.exists()) {
                ...
                if(data.userId === user.uid) {
                    ...
                    router.push('/ca-certificates/turnover-reconciliation');
                }
            } else {
      ...
      try {
          if (docId) {
              ...
              const docRef = await addDoc(collection(db, 'userDocuments'), {
                  userId: user.uid,
                  documentType: 'turnover-reconciliation-certificate',
              ...
              router.push(`/ca-certificates/turnover-reconciliation?id=${docRef.id}`);
          }
      } catch (e) {
          console.error(e);
  ...
  const handleLocalCertificationRequest = async () => {
      ...
      const basePrice = pricing?.ca_certs?.find(s => s.id === 'turnover_reconciliation')?.price || 0;
      ...
      if (effectivePrice === 0) {
          ...
          try {
            await addDoc(collection(db, "certificationRequests"), {
                reportType: "Turnover Reconciliation Certificate",
  ...
  const renderContent = () => {
    ...
    if (step === 1) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle>Turnover Reconciliation Information</CardTitle>
                </CardHeader>
                 <CardContent className="space-y-4">
                    ...
                    <Separator/>
                    <h4 className="font-semibold">Turnover Figures (₹)</h4>
                     <FormField control={form.control} name="gstr1Turnover" render={({ field }) => (<FormItem><FormLabel>GSTR-1 Turnover (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="gstr3bTurnover" render={({ field }) => (<FormItem><FormLabel>GSTR-3B Turnover (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="booksTurnover" render={({ field }) => (<FormItem><FormLabel>Books Turnover (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="variance" render={({ field }) => (<FormItem><FormLabel>Variance (₹) - Auto-calculated or manual</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="reasonForVariance" render={({ field }) => (<FormItem><FormLabel>Reason for Variance</FormLabel><FormControl><Textarea placeholder="e.g., Timing differences, export sales, advances, etc." {...field} /></FormControl><FormMessage /></FormItem>)}/>
                </CardContent>
    ...
    if (step === 2) {
        ...
        const whatsappMessage = `Dear ${formData.entityName},\n\nPlease find attached the Turnover Reconciliation Certificate for FY ${formData.financialYear}.\n\nThank you,\n${CA_FIRM.name}`;
             ...
             <Card>
                ...
                <CardContent>
                     <div ref={printRef} className="prose dark:prose-invert max-w-none border rounded-lg p-8 bg-card text-card-foreground">
                        ...
                        <h4 className="font-bold text-center underline my-6">TURNOVER RECONCILIATION CERTIFICATE</h4>

                        <p>This is to certify that we have verified the books of accounts, GST returns (GSTR-1 and GSTR-3B), and other relevant records of <strong>M/s {formData.entityName}</strong>{formData.fatherOrSpouseName ? ` (${formData.fatherOrSpouseName})` : ''}, having its registered office at {formData.entityAddress} and holding PAN <strong>{formData.entityPan}</strong>.</p>
                        
                        <p>Based on our verification for the financial year {formData.financialYear}, we certify that the turnover reconciliation is as follows:</p>
                        ...
                        <table className="my-6 w-full">
                            <tbody>
                                ...
                                <tr className="border-b">
                                    <td className="py-2">GSTR-1 Turnover</td>
                                    <td className="py-2 text-right font-mono">₹ {formData.gstr1Turnover.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                                <tr className="border-b">
                                    <td className="py-2">GSTR-3B Turnover</td>
                                    <td className="py-2 text-right font-mono">₹ {formData.gstr3bTurnover.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                                <tr className="border-b">
                                    <td className="py-2">Books Turnover</td>
                                    <td className="py-2 text-right font-mono">₹ {formData.booksTurnover.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                                <tr className="border-t-2 font-semibold">
                                    <td className="py-2">Variance</td>
                                    <td className="py-2 text-right font-mono">₹ {formData.variance.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                            </tbody>
                        </table>

                        <p><strong>Reason for Variance:</strong> {formData.reasonForVariance}</p>

                        <p className="mt-8 text-xs">
                            This certificate is issued at the specific request of the entity for the purpose of [Purpose, e.g., GST Audit, Bank Compliance]. The variance has been explained as above. Our liability is limited to the extent of information provided by the management and is based on the records produced before us.
                        </p>
                ...
                <CardFooter className="justify-between">
                     ...
                     <div className="flex gap-2">
                        <ShareButtons
                            contentRef={printRef}
                            fileName={`Turnover_Reconciliation_Certificate_${formData.entityName}`}
                        ...
                        {(() => {
                          ...
                          return effectivePrice > 0 ? (
                           <CashfreeCheckout
                               ...
                               planId="turnover_reconciliation_cert"
                               planName="Turnover Reconciliation Certificate"
                               userId={user?.uid || ''}
                               ...
                               postPaymentContext={{
                                 ...
                                 payload: {
                                   ...
                                   clientName: form.getValues("entityName"),
                                   documentType: "turnover-reconciliation-certificate",
                                   documentName: form.getValues("documentName") || "Turnover Reconciliation Certificate",
                                   formData: form.getValues(),
                                 },
                               }}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Turnover Reconciliation Certificate</h1>
        <p className="text-muted-foreground">Generate a draft certificate for turnover reconciliation across GSTR-1, GSTR-3B, and books.</p>
      </div>
      <Form {...form}>
