...
const formSchema = z.object({
  ...
  netProfitAsPerBooks: z.coerce.number(),
  depreciation: z.coerce.number().default(0),
...
export default function NetProfitCertificatePage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      documentName: `Net Profit Certificate - ${new Date().toISOString().split("T")[0]}`,
      entityName: "",
      fatherOrSpouseName: "",
      entityAddress: "",
      entityPan: "",
        ...
        const loadDocument = async () => {
            ...
            if (docSnap.exists()) {
                ...
                if(data.userId === user.uid) {
                    ...
                    router.push('/ca-certificates/net-profit');
                }
            } else {
      ...
      try {
          if (docId) {
              ...
              const docRef = await addDoc(collection(db, 'userDocuments'), {
                  userId: user.uid,
                  documentType: 'net-profit-certificate',
              ...
              router.push(`/ca-certificates/net-profit?id=${docRef.id}`);
          }
      } catch (e) {
          console.error(e);
  ...
  const handleLocalCertificationRequest = async () => {
      ...
      const basePrice = pricing?.ca_certs?.find(s => s.id === 'net_profit')?.price || 0;
      ...
      if (effectivePrice === 0) {
          ...
          try {
            await addDoc(collection(db, "certificationRequests"), {
                reportType: "Net Profit Certificate",
  ...
  const renderContent = () => {
    ...
    if (step === 1) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle>Net Profit Information</CardTitle>
                </CardHeader>
                 <CardContent className="space-y-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                     ...
                     <FormField control={form.control} name="netProfitAsPerBooks" render={({ field }) => (<FormItem><FormLabel>Net Profit as per Books (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="depreciation" render={({ field }) => (<FormItem><FormLabel>Depreciation (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="adjustments" render={({ field }) => (<FormItem><FormLabel>Adjustments (if any)</FormLabel><FormControl><Textarea placeholder="Enter any adjustments or notes" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                </CardContent>
    ...
    if (step === 2) {
        ...
        const whatsappMessage = `Dear ${formData.entityName},\n\nPlease find attached the Net Profit Certificate for FY ${formData.financialYear}.\n\nThank you,\n${CA_FIRM.name}`;
        const netProfit = formData.netProfitAsPerBooks;
             ...
             <Card>
                ...
                <CardContent>
                     <div ref={printRef} className="prose dark:prose-invert max-w-none border rounded-lg p-8 bg-card text-card-foreground">
                        ...
                        <h4 className="font-bold text-center underline my-6">NET PROFIT CERTIFICATE</h4>
                        ...
                        <p>Based on our verification of the books of accounts and financial statements for the financial year ended on 31st March {formData.financialYear.slice(0,4)}, we certify that the net profit of the entity is as follows:</p>
                        ...
                        <table className="my-6 w-full">
                            <tbody>
                                ...
                                <tr className="border-b">
                                    <td className="py-2 font-semibold">Net Profit as per Books</td>
                                    <td className="py-2 text-right font-mono font-semibold">₹ {netProfit.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                                {formData.depreciation > 0 && (
                                <tr className="border-b">
                                    <td className="py-2">Less: Depreciation</td>
                                    <td className="py-2 text-right font-mono">₹ {formData.depreciation.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                                )}
                                <tr className="border-b border-t-2">
                                    <td className="py-2 font-semibold">Net Profit (After Depreciation)</td>
                                    <td className="py-2 text-right font-mono font-semibold">₹ {(netProfit - (formData.depreciation || 0)).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                            </tbody>
                        </table>
                        
                        <p>The net profit is <strong>Rupees {numberToWords(netProfit - (formData.depreciation || 0))} only</strong>.</p>

                        {formData.adjustments && (
                            <p className="mt-4">Note: {formData.adjustments}</p>
                        )}

                        <p className="mt-8 text-xs">
                            This certificate is issued at the specific request of the entity for the purpose of submitting to [Purpose]. Our liability is limited to the extent of information provided by the management and is based on the records produced before us.
                        </p>
                ...
                <CardFooter className="justify-between">
                     ...
                     <div className="flex gap-2">
                        <ShareButtons
                            contentRef={printRef}
                            fileName={`Net_Profit_Certificate_${formData.entityName}`}
                        ...
                        {(() => {
                          ...
                          return effectivePrice > 0 ? (
                           <CashfreeCheckout
                               ...
                               planId="net_profit_cert"
                               planName="Net Profit Certificate"
                               userId={user?.uid || ''}
                               ...
                               postPaymentContext={{
                                 ...
                                 payload: {
                                   ...
                                   clientName: form.getValues("entityName"),
                                   documentType: "net-profit-certificate",
                                   documentName: form.getValues("documentName") || "Net Profit Certificate",
                                   formData: form.getValues(),
                                 },
                               }}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Net Profit Certificate</h1>
        <p className="text-muted-foreground">Generate a draft certificate for net profit as per books.</p>
      </div>
      <Form {...form}>
