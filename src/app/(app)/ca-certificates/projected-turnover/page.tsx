...
const formSchema = z.object({
  ...
  expectedTurnover: z.coerce.number().positive("Expected turnover must be a positive number."),
  basisOfCalculation: z.string().min(3, "Basis of calculation is required."),
});
...
export default function ProjectedTurnoverCertificatePage() {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      documentName: `Projected Turnover Certificate - ${new Date().toISOString().split("T")[0]}`,
      entityName: "",
      fatherOrSpouseName: "",
      entityAddress: "",
      entityPan: "",
      ...
      expectedTurnover: 0,
      basisOfCalculation: "based on past data and existing orders",
    },
  });
        ...
        const loadDocument = async () => {
            ...
            if (docSnap.exists()) {
                ...
                if(data.userId === user.uid) {
                    ...
                    router.push('/ca-certificates/projected-turnover');
                }
            } else {
      ...
      try {
          if (docId) {
              ...
              const docRef = await addDoc(collection(db, 'userDocuments'), {
                  userId: user.uid,
                  documentType: 'projected-turnover-certificate',
              ...
              router.push(`/ca-certificates/projected-turnover?id=${docRef.id}`);
          }
      } catch (e) {
          console.error(e);
  ...
  const handleLocalCertificationRequest = async () => {
      ...
      const basePrice = pricing?.ca_certs?.find(s => s.id === 'projected_turnover')?.price || 0;
      ...
      if (effectivePrice === 0) {
          ...
          try {
            await addDoc(collection(db, "certificationRequests"), {
                reportType: "Projected Turnover Certificate",
  ...
  const renderContent = () => {
    ...
    if (step === 1) {
        return (
            <Card>
                <CardHeader>
                    <CardTitle>Projected Turnover Information</CardTitle>
                </CardHeader>
                 <CardContent className="space-y-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                     ...
                     <FormField control={form.control} name="expectedTurnover" render={({ field }) => (<FormItem><FormLabel>Expected/Projected Turnover (₹)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                     <FormField control={form.control} name="basisOfCalculation" render={({ field }) => (<FormItem><FormLabel>Basis of Calculation</FormLabel><FormControl><Textarea placeholder="e.g., Based on past data and existing orders" {...field} /></FormControl><FormMessage /></FormItem>)}/>
                </CardContent>
    ...
    if (step === 2) {
        ...
        const whatsappMessage = `Dear ${formData.entityName},\n\nPlease find attached the Projected Turnover Certificate for FY ${formData.financialYear}.\n\nThank you,\n${CA_FIRM.name}`;
             ...
             <Card>
                ...
                <CardContent>
                     <div ref={printRef} className="prose dark:prose-invert max-w-none border rounded-lg p-8 bg-card text-card-foreground">
                        ...
                        <h4 className="font-bold text-center underline my-6">PROJECTED TURNOVER CERTIFICATE</h4>
                        ...
                        <p>Based on our review of the past performance and {formData.basisOfCalculation}, we certify that the projected/expected turnover of the entity for the financial year {formData.financialYear} is as follows:</p>
                        ...
                        <table className="my-6 w-full">
                            <tbody>
                                ...
                                <tr className="border-b">
                                    <td className="py-2 font-semibold">Projected / Expected Turnover</td>
                                    <td className="py-2 text-right font-mono font-semibold">₹ {formData.expectedTurnover.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                                </tr>
                            </tbody>
                        </table>
                        
                        <p>The projected turnover is <strong>Rupees {numberToWords(formData.expectedTurnover)} only</strong>.</p>

                        <p className="mt-8 text-xs">
                            This certificate is issued at the specific request of the entity for the purpose of submitting to [Purpose, e.g., Bank Loan Application, Tender]. Our liability is limited to the extent of information provided by the management and is based on the projections and assumptions as stated above.
                        </p>
                ...
                <CardFooter className="justify-between">
                     ...
                     <div className="flex gap-2">
                        <ShareButtons
                            contentRef={printRef}
                            fileName={`Projected_Turnover_Certificate_${formData.entityName}`}
                        ...
                        {(() => {
                          ...
                          return effectivePrice > 0 ? (
                           <CashfreeCheckout
                               ...
                               planId="projected_turnover_cert"
                               planName="Projected Turnover Certificate"
                               userId={user?.uid || ''}
                               ...
                               postPaymentContext={{
                                 ...
                                 payload: {
                                   ...
                                   clientName: form.getValues("entityName"),
                                   documentType: "projected-turnover-certificate",
                                   documentName: form.getValues("documentName") || "Projected Turnover Certificate",
                                   formData: form.getValues(),
                                 },
                               }}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        <h1 className="text-3xl font-bold">Projected Turnover Certificate</h1>
        <p className="text-muted-foreground">Generate a draft certificate for projected/expected turnover.</p>
      </div>
      <Form {...form}>
