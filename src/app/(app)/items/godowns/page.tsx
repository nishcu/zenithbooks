...
export default function GodownsPage() {
  ...
  const handleDelete = (id: string) => {
    ...
    console.log("Godown Deleted: The godown has been removed.");
  };

  const handleSave = () => {
    if (selectedGodown) {
        setGodowns(godowns.map(g => g.id === selectedGodown.id ? { ...g, ...formData } : g));
        console.log("Godown Updated: The godown details have been updated.");
    } else {
        const newGodown = { id: `loc-${Date.now()}`, ...formData };
        setGodowns([...godowns, newGodown]);
        console.log("Godown Added: A new godown has been created.");
    }
