...
export default function BalanceSheetPage() {
  ...
  const handleDownloadCsv = () => {
    ...
    // Create a more structured and printable CSV
    const rows = [
        // Header
        [`Balance Sheet as on ${formatDate}`],
        [""],
        // Liabilities Section
        ...
        ...liabilityExportRows.map(row => {
            if (row.type === 'header') {
                ...
                return [`${row.label}`, formatNumber(row.amount)];
            } else {
                return [`    ${row.label}`, formatNumber(row.amount)];
            }
        }),
        ...
        [""],
        // Footer
        ["Generated by ZenithBooks Accounting Software"],
        [`Report Date: ${formatDate}`],
        ...(isMismatched ? [[`Note: Balance sheet is mismatched by ₹${Math.abs(difference).toFixed(2)}`]] : [])
    ];
