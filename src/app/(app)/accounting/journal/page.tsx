...
export default function JournalVoucherPage() {
  ...
  const customers = useMemo(
    () =>
      customersSnapshot?.docs.map((doc: any) => ({
        value: doc.id,
        label: `${doc.data().name} (Customer)`,
        group: "Customers",
        ...doc.data(),
      })) || [],
    [customersSnapshot]
  ...
  const vendors = useMemo(
    () =>
      vendorsSnapshot?.docs.map((doc: any) => ({
        value: doc.id,
        label: `${doc.data().name} (Vendor)`,
        group: "Vendors",
        ...doc.data(),
      })) || [],
    [vendorsSnapshot]
  ...
  const handleDeleteJournalVoucher = async (voucherId: string) => {
    ...
    if (!originalVoucher) {
      enhancedToast({ variant: "destructive", title: "Error", description: "Original journal voucher not found." });
      return;
    }
  ...
  const handleVoucherAction = (action: string, voucher: JournalVoucher) => {
    if (action === "Delete") {
      if (voucher.reverses) {
        enhancedToast({ variant: "destructive", title: "Cannot Delete", description: "This is a reversal entry and cannot be deleted." });
        return;
      }
      ...
      if (voucher.reverses) {
        enhancedToast({ variant: "destructive", title: "Cannot Edit", description: "Reversal entries cannot be edited." });
        return;
      }
  ...
  const handleSaveVoucher = async () => {
    if (!date) {
      enhancedToast({ variant: "destructive", title: "Missing Details", description: "Please provide a date." });
      return;
    }
    ...
    try {
      ...
      enhancedToast({ variant: "destructive", title: "Save failed", description: e.message });
    }
  };
    ...
    <div className="space-y-8">
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={handleDialogClose}>
            ...
            <DialogContent className="max-w-4xl overflow-visible">
                          ...
                          <TableRow key={index}>
                            ...
                            <TableCell>
                              {showCostCentre && (
                                <Select
                                  ...
                                  onValueChange={(value: string) => handleLineChange(index, "costCentre", value)}
                                >
                                  <SelectTrigger>
                            ...
                            <TableCell>
                              <Input
                                ...
                                value={line.debit}
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleLineChange(index, "debit", e.target.value)}
                              />
                            </TableCell>
                            <TableCell>
                              <Input
                                ...
                                className="text-right"
                                value={line.credit}
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleLineChange(index, "credit", e.target.value)}
                              />
                            </TableCell>
              ...
              <DialogFooter>
                <Button onClick={handleSaveVoucher} disabled={!isBalanced}>
                  {editingVoucher ? "Save Changes" : "Save Voucher"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>A list of all manual journal entries. Reversed entries are hidden from this list.</CardDescription>
        ...
        <Dialog open={!!selectedVoucher} onOpenChange={(open: boolean) => !open && setSelectedVoucher(null)}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>Date: {format(new Date(selectedVoucher.date), "dd MMMM, yyyy")}</DialogDescription>
            </DialogHeader>
                  ...
                  {selectedVoucher.lines.map((line: any, index: number) => (
                    <TableRow key={index}>
                      <TableCell className="font-medium">
                        {combinedAccounts.find((a: any) => a.value === line.account)?.label || line.account}
                      </TableCell>
